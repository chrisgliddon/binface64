...
- Use project `assetExclusions` through `bf64 asset exclusion` for whole draft/reference trees.
- Use `toolchain install --dry-run` before bootstrapping Linux and `doctor --fix` to persist a validated SDK path atomically.

## Workflow

1. Run `./bf64 toolchain detect --json` and `./bf64 doctor --json`; if needed, preview `toolchain install --source <libdragon> --prefix <sdk> --dry-run --json` before executing it.
2. Create with `./bf64 new <dir> --name "<Name>" --json`; use `bf64 init --project <dir>` / `new --merge` for non-empty asset-first repositories, and `--force` only when replacing an intended target.
...
- `docs/docs/project/toolchain.md` for Linux discovery, install, and atomic project repair.
