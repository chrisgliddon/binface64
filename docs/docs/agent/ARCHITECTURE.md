...
**GOTCHA:** `assetManager.cpp:152` sets `entry.conf.baseScale = 16` as a default *before* loading the conf, but `format`/`baseScale`/`gltfBVH`/`exclude` are read with `doc[...]` (not `.value()`), so a **missing or malformed conf will throw** rather than fall back (lines 87-89, 97). Only `compression`/`wav*`/`font*` use safe defaults.
